...
"""
The neural network model simulation catapult dataset library.
"""
...
class TCatapultDatasetSimNN(TCatapultDataset):
  """
  Neural network model simulation catapult dataset controller.
  ...
  def __init__(self, abs_dirpath=None, verbose=True, auto_init=True):
    ...
    """
    super(TCatapultDatasetSimNN, self).__init__(abs_dirpath=abs_dirpath, verbose=verbose, auto_init=False)
    
    self._prefix = 'catapult_simNN/dataset'
  ...
  def new_entry_linear(self, *argv):
    raise Exception('This method is discarded. Use \"new_entry_linear_simNN\" instead.')

  def new_entry_linear_simNN(self, pos_init, pos_target, duration, loc_land):
    """
